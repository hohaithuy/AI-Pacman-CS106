...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining=0, catchExceptions=False, timeout=30):
    ...
    if (numGames-numTraining) > 0:
        ...
        #################### STORE RESULT ####################
        f = open("result.txt", "a")
        f.write(f'Average Score: {sum(scores) / float(len(scores))}\n')
        f.write(f'Mean Score:, {std(scores, sum(scores) / float(len(scores)))}\n')
        f.write(f'Win count: {wins.count(True)}\n')
        f.close()
        #################### STORE TIME ####################
...
if __name__ == '__main__':
    ...
    f.write(f'Time result: {time_result}\n')
